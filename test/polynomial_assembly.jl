...
@testset "Score loss assembly" begin
    ...
    univariateDiff2 = ntuple(j->reduce(hcat, k * (k-1) * pts[j,:] .^ (k-2) for k in 2:p)', d)
...
@testset "Check optimal coeffs" begin
    ...
    univariateDiff = ntuple(j->reduce(hcat, k * pts[j,:] .^ (k-1) for k in 1:p)', d)
    univariateDiff2 = ntuple(j->reduce(hcat, k * (k-1) * pts[j,:] .^ (k-2) for k in 2:p)', d)
    G,v = MultivariateExpansions.score_optimal_coeff(fmset, univariateEvals, univariateDiff, univariateDiff2)
    coeffs = G \ v
